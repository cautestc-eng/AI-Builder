...
import { verifyRequest, logAuthEvent } from "@/lib/auth";
...
export async function GET(req: NextRequest) {
  ...
  const { user, discordAccessToken } = verified;

  const guilds = await fetchGuilds(discordAccessToken);
  if (!guilds) {
  ...
  const owned = guilds.filter((g) => isOwner(g, user.id) || canManageGuild(g.permissions));
