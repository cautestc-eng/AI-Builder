...
export async function checkRateLimit(userId: string): Promise<{ allowed: boolean; limit: number; remaining: number }> {
  ...
  try {
    ...
    if (count === 0) {
      ...
      await supabase.from("rate_limits").update({ count: count + 1 }).eq("user_id", userId).eq("date", today);
    }

    return { allowed: true, limit: DAILY_LIMIT, remaining: DAILY_LIMIT - count - 1 };
  } catch {
    return { allowed: true, limit: DAILY_LIMIT, remaining: DAILY_LIMIT };
  }
}
